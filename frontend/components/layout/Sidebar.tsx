...
const NavItem = styled.button<{ $active?: boolean }>`
  ...
  padding: 8px 16px;
  border: none;
  background: ${(p) => (p.$active ? 'var(--nav-active-bg)' : 'transparent')};
  color: ${(p) => (p.$active ? 'var(--nav-blue-active)' : 'var(--nav-blue)')};
  font-size: 14px;
  ...
  &:hover {
    background: ${(p) => (p.$active ? 'var(--nav-active-bg)' : 'var(--surface-hover)')};
  }
`;
